...
export default function Services() {
  ...
  useGSAP(() => {
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        pin: true,
        scrub: 0.5, // Snappier, more responsive scroll feel
        start: "top top", 
        end: "+=100%", // Requires less physical scrolling to finish
        invalidateOnRefresh: true, 
      }
    });

    // SEQUENCE 1: Header moves up
    tl.to(headerRef.current, {
      y: "-15vh", 
      ease: "power2.out",
      duration: 1
    }, 0); // Trigger at exactly timeline 0

    // SEQUENCE 2: Cards slide up simultaneously, pushing the text
    tl.fromTo('.service-card', 
      { y: "60vh", opacity: 0 }, 
      {
        y: "0vh",
        opacity: 1,
        stagger: 0.05, // Faster ripple so they arrive tightly
        duration: 1, // Matches the header duration perfectly
        ease: "power2.out", // Matched easing curve of the header
      }, 
      0 // Trigger at exactly timeline 0 (No delay!)
    );
  ...
  return (
    <section 
      id="services" 
      ref={sectionRef} 
      className="relative w-full h-screen bg-black px-6 md:px-20 lg:px-24 flex flex-col items-center justify-center overflow-hidden cursor-default"
    >
      <div ref={headerRef} className="relative z-0 max-w-7xl mx-auto w-full text-center">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-center pt-10 pb-20 mt-10">
        {servicesData.map((service, index) => {
          const isHovered = hoveredIndex === index;
          const isAnotherHovered = hoveredIndex !== null && hoveredIndex !== index;

          // Break the dynamic classes into clean variables so they don't cause parsing errors
          const cardBaseClasses = "w-full h-full p-6 lg:p-8 pt-10 lg:pt-12 rounded-xl shadow-2xl transition-all duration-[600ms] ease-[cubic-bezier(0.34,1.56,0.64,1)]";
          
          const cardActiveClasses = isHovered 
            ? "-translate-y-12 rotate-0 scale-105 shadow-[0_30px_60px_rgba(0,0,0,0.8)]" 
            : `${service.rotation} ${isAnotherHovered ? "opacity-40 scale-95 translate-y-4" : "hover:scale-100"}`;
          ...
          return (
            <div
              ...
              className={`service-card relative md:-ml-8 lg:-ml-16 first:ml-0 transition-all duration-500 w-[90%] md:w-[220px] lg:w-[260px] flex-shrink-0 cursor-pointer ${isHovered ? 'z-50' : 'z-10'}`}
            >
              <div className={`${cardBaseClasses} ${service.colorClass} ${cardActiveClasses}`}>
                
                <div className={`absolute -top-10 -left-4 md:-left-6 transition-transform duration-500 ease-out ${isHovered ? 'scale-110 -rotate-12' : 'scale-100'}`}>
                  {service.sticker}
                </div>

                <div className="relative z-10 flex flex-col h-full mt-4">
                  <h3 className="text-2xl lg:text-3xl font-black tracking-tighter leading-none mb-4">
                    {service.title}
                  </h3>
                  
                  <div className="w-full h-[2px] bg-current opacity-30 mb-6" />
                  
                  <ul className="flex flex-col gap-3">
                    {service.items.map((item, i) => (
                      <li key={i} className="flex items-start gap-2 font-medium text-xs lg:text-sm">
